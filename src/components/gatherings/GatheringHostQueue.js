...
export default function GatheringHostQueue({ gatheringId, knockQuestion, onChanged }) {
  ...
  function openReport(userId, name) {
    setReportTarget({ userId, name: name || '申請人' });
    setReportReason('');
  }
  ...
  async function confirmReport() {
    if (!session?.access_token || !reportTarget?.userId) return;
    setReportBusy(true);
    setError('');
    try {
  ...
  const reportName = reportTarget?.name || '申請人';
  ...
  return (
    <>
          ...
          <li key={a.id} className="gathering-host-queue__item">
            <div className="gathering-host-queue__top">
              ...
              <button
                ...
                disabled={busyId === a.user_id || reportBusy}
                onClick={() => openReport(a.user_id, a.display_name)}
                title="舉報申請人"
      ...
      <GatheringConfirmOverlay
        open={!!reportTarget}
        title={`舉報申請人「${reportName}」？`}
        sub="月光守護者會收到通知並跟進。如涉及即時危險，請先報警。"
        confirmLabel="送出舉報"
        cancelLabel="返回"
        variant="danger"
        busy={reportBusy}
        ...
        noteLabel="舉報原因（選填）"
        notePlaceholder="簡述發生咩事，例如：騷擾、資料造假、不當內容…"
