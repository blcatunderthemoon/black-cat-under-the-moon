...
const ratelimit = process.env.UPSTASH_REDIS_REST_URL
  ? new Ratelimit({
      redis: Redis.fromEnv(),
      limiter: Ratelimit.slidingWindow(5, '1 h'), // 5 reports per hour per IP
    })
  : null;
...
export default async function handler(req, res) {
  ...
  if (ratelimit) {
    ...
    if (!success) {
      return res.status(429).json({ error: '檢舉太頻繁。' });
    }
  }
  ...
  try {
    ...
    if (reply_id) {
      ...
      const { error } = await supabase.rpc('increment_reply_report', { p_reply_id: reply_id });
      ...
      const { error } = await supabase.rpc('increment_report', { p_bottle_id: bottle_id });
      if (error) return res.status(500).json({ error: error.message });
    }
